...
class RuleDef:
    ...
    def eval_op(self, args: Optional[list[int]]) -> int:
    ...
    def confirm_cond(self, args: list[Any]) -> bool:
        # Case conditional rule
        if self.type != -1:
            return False

        data = self.eval_op(args)  # TODO invert op and update max/min
        cond = self.nbits.get(args)
...
class TimingsDef:
    ...
    def get_bit(self, value: int, args: Optional[list[int]]) -> list[int]:
        ...
        return [d.get(args) for d in signal]
